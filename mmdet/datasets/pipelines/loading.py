...
@PIPELINES.register_module()
class LoadMultiModalImageFromFiles:
    """Load multi-channel images from a list of separate channel files.
    ...
    def __call__(self, results):
        ...
        if "height" not in results['img_info']:
            ...
            for i, name in enumerate(filename):
                if 'visible' in name:
                    img_bytes = self.file_client.get(name)
                    img[:,:,0:3] = mmcv.imfrombytes(img_bytes, flag='color')
                else:
                    img_bytes = self.file_client.get(name)
                    img[:,:,3:] = mmcv.imfrombytes(img_bytes, flag='color')
        
        
        # img = np.stack(img, axis=-1) #[2, height, width, 3]
...
class LoadAnnotations:
    ...
    def __init__(self,
        ...
        self.with_illumination = with_illumination
    ...
    def _load_labels(self, results):
        ...
        return results
    def _load_illu_labels(self,results):
        # results['gt_illumination'] = np.array(np.argmax(np.bincount(results['ann_info']['illumination'])))
        results['gt_illumination'] = results['ann_info']['illumination']
        return results
    ...
    def __call__(self, results):
        ...
        if self.with_illumination:
            results = self._load_illu_labels(results)
        return results
