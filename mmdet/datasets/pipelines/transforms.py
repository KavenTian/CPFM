...
class Resize_Multi:
    ...
    def _resize_img(self, results):
        ...
        for key in results.get('img_fields', ['img']): 
            if self.keep_ratio:
                # TODO:保持长宽比缩放图像
                img_shape = (results['scale'][1],results['scale'][0], results[key].shape[-1])    #
                img = np.zeros(shape=img_shape, dtype=np.float32)
                if img_shape[-1] == 4:  # height, width, channel
                        img[:, :, 0:3], scale_factor = mmcv.imrescale(
                                                results[key][:, :, 0:3],
                                                results['scale'],
                                                return_scale=True,
                                                backend=self.backend)
                        img[:, :, 3], scale_factor = mmcv.imrescale(
                                                results[key][:, :, 3],
                                                results['scale'],
                                                return_scale=True,
                                                backend=self.backend)
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: Resized results, 'img_shape', 'pad_shape', 'scale_factor', \
                'keep_ratio' keys are added into result dict.
        """
        # 前两行代码仅用于指定scale方法有效，否则需删除
        if 'scale_factor' in results:
            results.pop('scale_factor')
        if 'scale' not in results:
            if 'scale_factor' in results:   #scale_factor:1.0
                img_shape = results['img'].shape[:2]
                scale_factor = results['scale_factor']
                assert isinstance(scale_factor, float)
                results['scale'] = tuple(
        ...
        for key in results.get('img_fields', ['img']):
            ...
            if self.size is not None:
                if results['img'].shape[-1] > 3:
                    padded_img = np.zeros((self.size[0], self.size[1], results['img'].shape[-1]))
                    padded_img[:, :, 0:3] = mmcv.impad(
                    results[key][:, :, 0:3], shape=self.size, pad_val=pad_val)
                    padded_img[:, :, 3:] = mmcv.impad(
                    results[key][:, :, 3:], shape=self.size, pad_val=pad_val)
                else:
...
class Mosaic:
    ...
    def _mosaic_transform(self, results):
        ...
        if len(results['img'].shape) == 3:
            mosaic_img = np.full(
                (int(self.img_scale[0] * 2), int(self.img_scale[1] * 2), ch),
                self.pad_val,
...
class MixUp:
    ...
    def __call__(self, results):
        ...
        org_results = results
        results = self._mixup_transform(org_results)
        # while results["gt_bboxes"].shape[0] == 0:
        #     results = self._mixup_transform(org_results)
    ...
    def _mixup_transform(self, results):
        ...
        if len(retrieve_img.shape) == 3:
            out_img = np.ones(
                (self.dynamic_scale[0], self.dynamic_scale[1], ch),
